...
"ex_ez_pbe0_soc_temp1":"sp_e_static",
"grd_at_ex_cz_pbe0_soc":"grd@sp_c",
"grd_at_ex_ez_pbe0":"grd@sp_e_w/o_soc",
"grd_at_ex_ez_pbe0_soc":"grd@sp_e",
"grd_at_ex_cb_pbe0_soc":"grd@cb",
"grd_at_ex_vb_pbe0_soc":"grd@vb",
"relax_ex_cb_pbe0_soc":"cb",
"relax_ex_cz_pbe0_soc":"sp_c",
"relax_ex_ez_pbe0":"sp_e_w/o_soc",
"relax_ex_ez_pbe0_soc":"sp_e",
...
"relax_ex_cz_half_pbe0_soc":"sp_c_half",
"relax_ex_ez_half_pbe0_soc":"sp_e_half",
            ...
            for j in line_dict:
                if j in i:
                    for k in line_dict[j]:
                        if k:
                            if i.replace(*k) in energy_dict and type(energy_dict[i.replace(*k)])==tuple:
                                pos=list(zip(energy_dict[i],energy_dict[i.replace(*k)]))
                                if abs(pos[0][0]-pos[0][1])<1e-3 or abs(pos[1][0]-pos[1][1])<3:
                                    ax.plot(*pos,'ro--')
                                    ax2.plot(*pos,'ro--')
                                else:
                                ...
                                if 0.4<(pos[1][0]+pos[1][1])/2<3.6:
                                    if False:
                                        ax.annotate(str(round(abs(energy_dict[i.replace(*k)][1]-energy_dict[i][1]),2))+"eV",
                                                 xy=((pos[0][0]+pos[0][1])/2,((pos[1][0]+pos[1][1])/2-2)/8+3.6),xytext=(0,0),
                                                 textcoords='offset points',fontsize=word_size)
                                        ax2.annotate(str(round(abs(energy_dict[i.replace(*k)][1]-energy_dict[i][1]),2))+"eV",
                                                 xy=((pos[0][0]+pos[0][1])/2,((pos[1][0]+pos[1][1])/2-2)/8+3.6),xytext=(0,0),
                                                 textcoords='offset points',fontsize=word_size)
                                    else:
                                        ax.annotate(str(round(abs(energy_dict[i.replace(*k)][1]-energy_dict[i][1]),2))+"eV",
                                                 xy=((pos[0][0]+pos[0][1])/2,((pos[1][0]+pos[1][1])/2-2)/8+0.4),xytext=(0,0),
                                                 textcoords='offset points',fontsize=word_size)
                                        ax2.annotate(str(round(abs(energy_dict[i.replace(*k)][1]-energy_dict[i][1]),2))+"eV",
                                                 xy=((pos[0][0]+pos[0][1])/2,((pos[1][0]+pos[1][1])/2-2)/8+0.4),xytext=(0,0),
                                                 textcoords='offset points',fontsize=word_size)
                    ...
                    for k in fit_dict[j]:
                        if k:
                            if i.replace(*k) in energy_dict and type(energy_dict[i.replace(*k)])==tuple:
                                ...
                                ax2.plot(x,np.poly1d(coef)(x))
            ax.plot(*energy_dict[i],"rx")
            ax2.plot(*energy_dict[i],"rx")
            ax.annotate(replace_dict[i],xy=energy_dict[i],xytext=(0,0),textcoords='offset points',fontsize=word_size)
            ax2.annotate(replace_dict[i],xy=energy_dict[i],xytext=(0,0),textcoords='offset points',fontsize=word_size)
    
    ax2.set_ylim(-0.25,0.5)
    ax.set_ylim(3.5,4.25)
...
"Cs2KInCl6c2h":-5.515516321}
    ...
    for i in range(len(line_list)):
        for j in range(len(line_list[i])):
            if "VBM" in line_list[i][j][2]:
                ...
                plt.plot([i*interval,i*interval+length],[line_list[i][j][0],line_list[i][j][0]],color="cyan",linestyle="--",linewidth=1)
                # if line_list[i][j][2]:
    ...
    plt.xticks(np.linspace(0,len(split_list),len(split_list)+1)[:-1]*interval+0.5,
               [i["system"].replace("1Cs2","").replace("Cl6","").replace("_Bi","").replace("_Sb","") for i in split_list])
    plt.ylabel('energy (eV)')
    plt.xlim(-0.25,11.75)
    plt.ylim(-9,1)
    #plt.title("ctl")
...
def plot_split(split_list,path):
    ...
    line_list=[[[0,         [0,1,2,3,4,5],  "p",                                                                [10,10]]],
                ...
                if i.detail["type"]=="matproj":
                    ...
                    for j in self.job_path_list:
                        if j in [system+"/"+k 
                            ...
                            p.read(j+"/CONTCAR")
                            energy_dict1[j.split("/")[-1]]=(p.cc_xtick(i.detail["type"]),energy(j))
                            ...
                            if j.split("/")[-1]=="grdm1_pbe0_soc":
                                ...
                                p.read(j+"/CONTCAR")
                                energy_dict1[j.split("/")[-1]]=(p.cc_xtick(i.detail["type"]),{})
                                for k in procar:
                                    if k["occ"]<0.4:
                                        break
                                    elif k["occ"]<0.6:
