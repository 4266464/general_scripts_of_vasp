...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--type", default="abs", choices=["abs", "select"], help="select the reading type")
    parser.add_argument("-b","--bands",type=int, default=15,help="how many bands included in 'abs' type")
    parser.add_argument("-d", "--detail", default=[],nargs="+",help = "the detailed setting of 'select' type")
    ...
    if args.mode=="abs":
        ...
        for i in args.detail:
            a,b,c=i.split("_")
            _dict[(a,b)]=[int(c)]
        for i in range(len(procar[0]))[num:]:
            for j in procar:
                for k in _dict:
                    if k in j[i]["spd"] and _dict[k][0]>= len(_dict[k]):
                        _dict[k].append(j[i])
                        break
            # if all(_dict.values()):
            #     break
