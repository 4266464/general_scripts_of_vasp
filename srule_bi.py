...
def scatter_plot_ex(list_scat):
    ...
    for j,k in zip(range(6)[2:],["grd","m1","p1","sp"]):
        for i in list_scat:
            if i[j]:
                plt.scatter(i[1],i[j],color="red",s=1)
                plt.annotate(i[0].replace("Cs2","").replace("Cl6","").split("_")[0]+"_"+k,xy=(i[1],i[j]),xytext=(0,0),
    ...
    plt.plot([2.4,3.2],[2.4,3.2],linestyle=(0,(1,10)),color="black")
    #plt.xlabel('pri')
    plt.xlim(2.4,3.2)
    plt.ylim(2.4,3.2)
    #plt.ylabel(k)
    plt.show()
    #plt.savefig("1001b",dpi=300)
    plt.close("all")
