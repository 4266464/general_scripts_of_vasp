...
class Wavecar(object):
    ...
    def read_coef(self, spin_index=1, kpoint_index=1, band_index=1):
        """
    ...
    def wfc_r(self, spin_index=1, kpoint_index=1, band_index=1,
              fft_grid=None, kr_phase=False, r_shift=[0.0, 0.0, 0.0]):
        """
        ...
        # \sum_{ijk} | \phi_{ijk} | ^ 2 = 1
        # if phi is well normalized, fftn(phi) should divide np.sqrt(np.prod(fft_grid))
        # and ifftn(phi) should multiply np.sqrt(np.prod(fft_grid))
        # normalization_factor = np.sqrt(np.prod(fft_grid))
        normalization_factor = np.prod(fft_grid) / np.sqrt(self.omega)
        ...
        dump = self.read_coef(spin_index, kpoint_index, band_index)
        ...
        if self.soc:
            ...
            if self.gamma:
                ...
                if self.gamma_half == 'z':
                    ...
                    tmp = np.fft.irfftn(phi_k, s=(fft_grid[2], fft_grid[1], fft_grid[0])) * normalization_factor
    ...
    def poisson(self, rho=None, band_index=1, kpoint_index=1, spin_index=1, fft_grid=None):
        """
        ...
        if rho is not None:
            ...
            # normalization_factor = np.prod(fft_grid) / self.omega
            if self.soc:
                ...
                for phi in phi_spinor:
                    rho += -(phi.conj() * phi).real # * normalization_factor
            else:
            ...
            for kpoint_index in range(self.nkpoints):
                ...
                for band_index in range(self.nbands):
                    ...
                    phi_r = self.wfc_r(spin_index=spin_index + 1, kpoint_index=kpoint_index + 1,
                                       band_index=band_index + 1, fft_grid=fft_grid)  # * normalization_factor
...
if __name__ == '__main__':
    ...
    for i in [637, 638, 639]:
        phi = wf.wfc_r(spin_index=1, kpoint_index=1, band_index=i, kr_phase=False)
        print(np.linalg.norm(phi)*np.sqrt(wf.omega/np.prod(wf.fft_grid)))
