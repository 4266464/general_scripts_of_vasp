...
from chempot_old import list2formula, formula2list
from material_project import convert_formula_to_table
from material_project import material_project_repo
...
def construct_line_with_two_points(point1, point2):
    return [(point1[1] - point2[1]) / 1e-5, (point1[0] * point2[1] - point2[0] * point1[1]) / 1e-5] \
        if abs(point1[0] - point2[0]) < 1e-5 else \
        [(point1[1] - point2[1]) / (point1[0] - point2[0]),
         (point1[0] * point2[1] - point2[0] * point1[1]) / (point1[0] - point2[0])]


def crossing_point(line1, line2):
    return [-(line1[1] - line2[1]) / 1e-5, (line1[0] * line2[1] - line2[0] * line1[1]) / 1e-5] \
        if abs(line1[0] - line2[0]) < 1e-5 else \
        [-(line1[1] - line2[1]) / (line1[0] - line2[0]),
...
def chempot_plot_sample(formula, fixed={}, exclude=[]):
    ...
    points[tuple(crossing_point([0, 0], [1e5, 0]))] = [f"{order[1]}=0", f"{order[2]}=0"]
    points[tuple(crossing_point([0, 0], line_background))] = [f"{order[0]}=0", f"{order[2]}=0"]
    points[tuple(crossing_point(line_background, [1e5, 0]))] = [f"{order[0]}=0", f"{order[1]}=0"]
    ...
    frame = {}
    frame[f"{order[0]}=0"] = [crossing_point([0, 0], line_background), crossing_point(line_background, [1e5, 0])]
    frame[f"{order[1]}=0"] = [crossing_point([0, 0], [1e5, 0]), crossing_point([1e5, 0], line_background)]
    frame[f"{order[2]}=0"] = [crossing_point([1e5, 0], [0, 0]), crossing_point([0, 0], line_background)]
                ...
                for k in points[j]:
                    for l in points:
                        ...
                        if k in points[l] and l not in bad_points:
                            # 求交点
                            points[tuple(crossing_point(lines[k], elimi))] = [i[-1], k]
                            break
                del points[j]
            lines[i[-1]] = elimi
    ...
    for i in tobeplotted:
        _points = []
        for j in frame:
            _point = crossing_point(construct_line_with_two_points(frame[j][0], frame[j][1]), lines[i])
            # 判断交点是否在图形上
...
if __name__ == "__main__":
    # print(formula2tuple("Cs2NaBiCl6"))
    # tuple2formula(chem_tuple)
    # chempot_sample("CaAl12O19",sample_times=0,other_element=["Fe", "Bi", "Pb"],start_point=[-2.317373553,-3.065047363,-4.16613],exclude=["CaAl4O7"])
    # chempot_sample("Cs2HfCl6", sample_times=10000, other_element=["Sb", "Bi", "Se", "Te", "Zr"])
    # chempot_sample("Y3Al5O12",fixed={"O":-1.3},other_element=["Ca", "Be", "Mg", "Sr", "Na", "Li", "Ti", "Ge", "Si"],sample_times=10000)
    # chempot_sample("Mg2Al4Si5O18",other_element=["Cr"], fixed={"O":-2.4},sample_times=10000)
    # chempot_sample("RbMgF3",other_element=["Cu"],sample_times=10000)
    # chempot_sample("SrB4O7",sample_times=10000)
    # chempot_sample("YPO4", sample_times=1000000, other_element=["Sb"])
    # chempot_sample("Mg2SnO4",sample_times=10000, other_element=["Li"])
    # chempot_sample("La3Ga5SnO14",sample_times=10000)
    # other_element=["Sc","Ti","V","Cr","Mn","Fe","Co","Ni","Cu","Zn"]
    # sample=False
    ...
    # chempot_plot_sample("Y2Ti2O7",exclude=["Y2TiO5"])
    # chempot_plot("Mg2Al4Si5O18", fixed={"O":-4.9})
    # chempot_plot_sample("Ca5Ga6O14")
    # chempot_sample("Cs2HfCl6", sample_times=10000)
    chempot_sample("Y2Ti2O7", sample_times=10000)
