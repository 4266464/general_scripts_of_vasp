...
class poscar:
    ...
    def read(self, path):
        ...
        with open(path, "r") as f:
            for i in f.readlines():
                lines.append([(float(j) if j[1:].replace(".","").isdigit() else j) for j in i.replace("\n", "").split(" ") if j])
        ...
        self.position = []
        if lines[7][0] == "Selective":
            self.selective = []
            self.mode = lines[8][0]
            for i in lines[9:]:
                if i:
                    self.position.append(np.array(i[:3]))
                    self.selective.append(i[3:])
                else:
                    break
        else:
            ...
            for i in lines[8:]:
                if i:
                    self.position.append(np.array(i))
                else:
                    break
        return None
    ...
    def write(self, path):
        ...
        if self.selective:
            string += "Selective\n"
            string += f"{self.mode}\n"
            for i, j in zip(self.position, self.selective):
                for k in i:
                    string += f"{k} "
                for k in j:
                    string += f"{k} "
    ...
    def __mul__(self, multiplier):
        aposcar = self.copy()
        aposcar.position = [i * multiplier for i in self.position]
        return aposcar
        ...
        for i, j in zip(self.label, self.position):
            ...
            if full:
                distance_list.append([i, np.array(
                    [k if abs(k) < 0.5 else (k + 1 if k < 0 else k - 1) for k in (j - site_position)]).dot(self.axis)])
            else:
                distance_list.append([i, np.linalg.norm(
                    np.array([k if abs(k) < 0.5 else (k + 1 if k < 0 else k - 1) for k in (j - site_position)]).dot(
                        self.axis))])
...
class job:
    ...
    def read_job(self,string):
        ...
        template_dict={
            ...
            "formation":["type","formula","defect_list","host"]}
        self.detail={}
        ...
        for i in self.job_list:
                            ...
                            if i.bundle:
                                ...
                                for j in os.listdir(i.detail["structure"]):
                                    ...
                                    if self.job_list[-1].detail["wavecar"]:
                                        self.job_list[-1].detail["wavecar"]=self.job_list[-1].detail["wavecar"]+"/"+j
            ...
            # 非vasp
                ...
                if i.detail["type"]=="matproj":
                    ...
                    # {(-0.01639676565961838, -1.9242244802322939, -2.776211249455967, -2.304371136448475):
                    # {'tag':['Bi_max'], 
                    # 'chempot':
                    # {'Bi':-4.714431075659618, 
                    # 'Cl':-3.1587611402322935, 
                    # 'Cs':-3.6716136215249326, 
                    # 'Na':-3.6268964298967505}}}
                    
                    pot={"Cs":0.388,"Na":0.142,"Bi":0.303,"Cl":0.15,"vac":0,"O":0.059,"Sn":0.294,"Ag":0.189,"In":0.275,"Sb":0.29}
                    #pot={"Cs":0.271,"Bi":0.208,"Cl":0.1,"vac":0}
                    # for j in pot:
                    #     pot[j]/=4
                    
                    energy_dict={}
                    ajob=self.find_job_by_path(i.detail["host"])
                    ...
                    for j in procar:
                        if j["occ"]<0.3:
                            ...
                            break
                    grd_energy=energy(ajob.detail["path"])-charge_corr_dict[ajob.detail["path"].split("/")[0]]*ajob.detail["nele"]**2
                    for j in self.job_list:
                        for l in i.detail["defect_list"].split("-"):
                            if j.detail["type"]=="vasp" and ajob.detail["path"].split("/")[0]+"/relax_"+l in j.detail["path"]:
                                ...
                                if not energy_dict or not j.detail["path"].split("_")[1] in energy_dict:
                                    energy_dict[j.detail["path"].split("_")[1]]=[]
                                ...
                                temp_energy_dict["img"]=temp_energy_dict["madebula"]*temp_energy_dict["qq"]*temp_energy_dict["qq"]
                                
                                temp_energy_dict["energy"]=temp_energy_dict["defect"]-temp_energy_dict["host"]+temp_energy_dict["img"]+temp_energy_dict["potalign"]
                                ...
                                print(temp_str1)
                                energy_dict[j.detail["path"].split("_")[1]].append([-j.detail["nele"],temp_energy_dict["energy"],j.detail["path"].split("_")[1],
                                                                                   origin_site,new_site])
                    
                    print(energy_dict)
                    for k in cp:
                        plot_ctl5([vbm,cbm],
                                  [[[l[0],l[1]+(sum([k["chempot"][m] for m in l[3]])-sum([k["chempot"][m] for m in l[4]])),l[2]] for l in j] for j in energy_dict.values()],
                                  0.05,
                                  i.detail["formula"]+"_".join(k["tag"]))
