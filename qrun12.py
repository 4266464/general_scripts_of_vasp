                            ...
                            if i.replace(*k[:2]) in energy_dict and type(energy_dict[i.replace(*k[:2])]) == tuple:
                                ...
                                if "relax_" in i and "relax_" in i.replace(*k[:2]):
                                    ...
                                    if i.replace("relax_", "")[:3] == i.replace(*k[:2]).replace("relax_", "")[:3]:
                                        ...
                                        plt.plot(*pos, linestyle="dashed", marker="o",
                                                 color=k[2] if len(k) == 3 else "red")
                                        ...
                                        plt.annotate(temp,
                                                     xy=((pos[0][0] + pos[0][1]) / 2, (pos[1][0] + pos[1][1]) / 2),
                                                     xytext=(a, 10),
            ...
            if not "at" in i:
                if energy_dict["relax_grd_pbe0_soc"][0] > 1:
                    ...
                    if "cz" in i:
                        ...
                        a = -10
                plt.annotate(replace_dict[i] if not replace_dict[i] == "grd" else "", xy=energy_dict[i],
                             xytext=(a, 30 if "relax_grd" in i else 10), textcoords='offset points', fontsize=14)
    plt.annotate(path.split("_")[-1], xy=(xmax, ymax + 0.12 * (ymax - ymin)), xytext=(0, 0), textcoords='offset points',
                 fontsize=14)
    ax = plt.gca()
    if energy_dict["relax_grd_pbe0_soc"][0] > 1:
    ...
    for i in range(len(line_list)):
        for j in range(len(line_list[i])):
            ...
            if line_list[i][j][2] and not "eV" in line_list[i][j][2]:
                plt.annotate(line_list[i][j][2], xy=(i * interval, line_list[i][j][0]), xytext=line_list[i][j][3],
                             textcoords='offset points', fontsize=14)
            if not i == len(line_list) - 1:
                for k in line_list[i][j][1]:
                    plt.plot([i * interval + length, (i + 1) * interval], [line_list[i][j][0], line_list[i + 1][k][0]],
...
class job:
    ...
    def read_job(self, string):
        ...
        template_dict = {
            ...
            "formation": ["type", "chemical", "dielectric", "defect_list", "host1", "host2", "name"]}
        self.detail = {}
    ...
    def runvasp(self, tag="normal"):
        ...
        if hostname == "tc4600v3":
            ...
            blist = ["#!/bin/bash",
                     ...
                     "#SBATCH -o %j.log",
                     "#SBATCH --qos=cpu",
                     "mpirun /share/softwares/packages/vasp.5.4.4_oneAPI/bin/"]
            if self.detail["soc"]:
                blist[-1] += "vasp_ncl"
            elif self.detail["kmesh"] == "gamma":
                blist[-1] += "vasp_gam"
    ...
    def update_vasp(self, path):
        ...
        if "jobid" in os.listdir(path):
            ...
            if hostname == "tc4600v3":
                ...
                # 没有log文件
                if jobid in [[j for j in i.split(" ") if j][0] for i in
                             scc_lib.linux_command("squeue -u ckduan -o '%.8i  %.1D  %8T %8R %.10M   %20V %12l %Z'")[
                             1:]]:
        ...
        for i in self.job_list:
            ...
            # 非vasp
                ...
                if i.detail["type"] == "matproj":
                    ...
                    cp = []
                    with open(i.detail["chemical"], "r") as f:
                        for j in f.readlines():
                    ...
                    for k in cp:
                        ...
                        print(i.detail["name"] + i.detail["chemical"] + "_".join(k["tag"]))
                        print(charged_energy_dict)

                        pctl.plot_ctl(charged_energy_dict,
                                      150,
                                      i.detail["name"] + i.detail["chemical"] + "_".join(k["tag"]),
                                      [vbm1, cbm1, 100])
                    i.state = "done"
                ...
                elif i.detail["type"] == "corr":
                    if os.path.isdir(i.detail["path"]):

                        pass
                        i.state = "done"

                    elif self.find_job_by_path(i.detail["dielectric"]).state == "done":
                        scc_lib.linux_command(
                            f"mkdir {i.detail['path']};"
                            f"cp {i.detail['dielectric']}/CONTCAR {i.detail['path']}/CONTCAR;"
                            f"cp {i.detail['dielectric']}/OUTCAR {i.detail['path']}/OUTCAR")
                        os.chdir(i.detail['path'])
                        scc_lib.linux_command(f"bsub -q test -o %J.log python charge_corr.py")
                        os.chdir("../")
                        i.state = "run"
