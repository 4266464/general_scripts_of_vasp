        ...
        with open(formula+"_".join([i+"_"+str(round(fixed[i],3)) for i in fixed])+".csv","w") as f:
            ...
            if chempot_list:
                ...
                for i in res_dict:
                    ...
                    ax2.scatter(*res_dict[i][-1],color="blue")
                    ax2.annotate(",".join(res_dict[i][:-3]),res_dict[i][-1],xytext=(0,0),textcoords='offset points',fontsize=8)
                ...
                if len(real_formula_tuple)==3:
                    ...
                    temp=[[i,res_dict[i][-1]] for i in res_dict]
...
#chempot_sample("MgAl2O4",other_element=["Cl"])
